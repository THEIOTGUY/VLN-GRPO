...
class BatchEnvServer:
    ...
    def _setup_routes(self):
        ...
        def health_check():
            return jsonify({"status": "ok", "message": "Environment server is running"}), 200

        @self.app.route('/admin/reset_all', methods=['POST'])
        def admin_reset_all():
            """Reset all active environments and release their simulator resources."""
            ray.get(self.service.close_batch.remote(None))
            return jsonify({"message": "All environments reset"}), 200
