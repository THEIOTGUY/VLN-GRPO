...
class VLNCEEnvConfig:
    ...
    # When True, the env checks the depth sensor after every physical action
    # and flags replan_triggered=True in the info dict when an obstacle is
    # detected within obstacle_depth_threshold metres in the central field of view.
    enable_obstacle_replanning: bool = False
    obstacle_depth_threshold: float = 0.8  # metres

    # --- Spatial memory module ---
    # When True, a SpatialMemory object tracks visited positions and landmark
    # discoveries and appends a text summary to every observation.
    enable_memory_module: bool = False
    memory_grid_resolution: float = 0.25  # metres per grid cell

    # --- Semantic landmark reward ---
    # When True, the agent receives landmark_reward_base bonus per newly
    # discovered named landmark (proximity to GT waypoints used as proxy).
    enable_landmark_reward: bool = False
    landmark_reward_base: float = 1.0
    landmark_proximity_threshold: float = 2.0  # metres
