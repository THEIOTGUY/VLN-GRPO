...
def init_observation_template(observation, instruction, memory_summary: str = ""):
    mem = f"\n{memory_summary}" if memory_summary else ""
    return f"""[Initial Observation]:
{observation}
Instruction: {instruction}{mem}
Decide your next action. """


def action_template(observation, instruction, memory_summary: str = "", replan: bool = False):
    prefix = "[Replanning — obstacle detected]" if replan else "After that, the observation is:"
    mem = f"\n{memory_summary}" if memory_summary else ""
    return f"""{prefix}
{observation}
Instruction: {instruction}{mem}
